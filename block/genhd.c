...
static void __disk_unblock_events(struct gendisk *disk, bool check_now)
{
	...
	if (check_now)
		queue_delayed_work(system_nrt_freezable_power_efficient_wq, &ev->dwork, 0);
	else if (intv)
		queue_delayed_work(system_nrt_freezable_power_efficient_wq, &ev->dwork, intv);
out_unlock:
	spin_unlock_irqrestore(&ev->lock, flags);
}
...
void disk_flush_events(struct gendisk *disk, unsigned int mask)
{
	...
	if (!ev->block) {
		...
		queue_delayed_work(system_nrt_power_efficient_freezable_wq, &ev->dwork, 0);
	}
